...
router.put("/:id", async (req, res) => {
  if (String(req.body.userId) === String(req.params.id) || req.body.isAdmin) {
    ...
    if (req.body.coverPicture)
      updatedFields.coverPicture = req.body.coverPicture;

    try {
      // Find the current user
      const user = await User.findById(req.params.id);

      // After checking for new photo, add this for removal:
      if (
      ...
      if (
        ...
        try {
          await deleteFile(user.coverPicture);
        } catch (err) {
          console.warn("Failed to delete old cover picture from S3:", err);
        }
      }
